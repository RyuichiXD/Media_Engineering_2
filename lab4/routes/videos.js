...
//This will get the Filter-Middleware, where are the functions for ?filter, ?limit and ?offset are
const filter = require('../restapi/filter');
...
videos.route('/')
    .get(function (req, res, next) {
        ...
        if (typeof videos !== 'undefined') {
            if (typeof req.query.filter === 'undefined') {
                ...
                newVideos = videos.map(function (video) {
                    try {
                        var newVideo = filter.filterVideo(filterOptions, video);
                        return newVideo;
                    } catch (e) {
                        return next(e);
                    }
        ...
        try {
            res.locals.items = filter.getOffset(req.query.offset, res.locals.items);
            res.locals.items = filter.getLimit(req.query.limit, res.locals.items);
        } catch(e) {
            return next(e);
        }
